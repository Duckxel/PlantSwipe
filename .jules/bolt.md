...
**Learning:** Performing multiple \`.reduce()\` or chained \`.filter().length\` operations over the same array to calculate related metrics (e.g., total required tasks and total completed tasks) increases time complexity to O(2N) and adds unnecessary function call overhead.
**Action:** Replace multiple \`.reduce()\` calls or \`.filter().length\` chains on the same array with a single-pass \`for\` loop that computes all required aggregates simultaneously, avoiding intermediate array allocations and reducing loop overhead.
## 2026-05-03 - Array Initialization in Hot Paths
...
## 2024-05-15 - Optimizing Array Processing in React Rendering Paths
**Learning:** In React components like `AdminPlantDumpPage`, chaining array methods like `.filter().length` multiple times over the same large array (e.g., to count different statuses) inside the render path causes O(k*N) iteration and unnecessary intermediate array allocations, increasing garbage collection pressure.
**Action:** Replace chained multiple `.filter().length` calculations with a single-pass `for` loop `O(N)` that tallies all required metrics simultaneously. Ensure to prefix the optimization with a `// ⚡ Bolt:` comment explaining the change.
