...
## 2024-05-16 - Duplicated UI Patterns in Large Files Often Drop Accessibility Attributes
**Learning:** In complex interface panels (like `AdminLandingPanel.tsx`), when list items or iterated data components duplicate similar UI controls (such as 'move up/down', 'edit/delete', or 'close' buttons) from earlier sections, the duplicated controls frequently omit `aria-label` and `title` attributes that were correctly implemented on the initial instances. This creates an inconsistent accessibility experience where similar looking buttons behave differently for screen readers.
**Action:** When adding accessibility attributes to a specific component type (like an icon-only `<Button size="icon">`), systematically scan the entire file (e.g., using multiline `grep` or search features) to ensure all equivalent iterations of that control pattern receive the same accessibility treatment, rather than relying on a single instance search.
