...
## 2026-04-02 - Keyboard Accessibility for Custom Toggle Buttons
**Learning:** Custom toggle buttons built with elements like `div` or customized `button` elements (e.g., in Garden Editors for Climate, Living Space, and Usage) often miss explicit focus styles. Additionally, elements like `motion.div` acting as tooltips (e.g., in Profile Badges) cannot be accessed via keyboard navigation without `tabIndex={0}` and focus handlers. Both require `focus-visible` to support keyboard navigation fully.
**Action:** Always add `focus-visible:outline-none focus-visible:ring-2` (and an appropriate ring color) to custom toggle buttons. For interactive elements like tooltips that are not native buttons, change them to native buttons or add `tabIndex={0}` along with `onFocus` and `onBlur` handlers so screen reader and keyboard users can trigger them.
