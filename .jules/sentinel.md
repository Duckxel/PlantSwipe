...
## 2025-03-08 - Missing Authentication on Additional Admin Lookup Endpoints (/api/admin/members-by-ip)
**Vulnerability:** The `/api/admin/members-by-ip` endpoint in `server.js` was using `isAdminFromRequest(req)` to check if the caller was an admin, but it failed to properly authenticate the request and return standard 401 Unauthorized responses for unauthenticated requests, unlike most other administrative endpoints that use `ensureAdmin(req, res)`.
**Learning:** Checking for administrative privileges directly using boolean helpers like `isAdminFromRequest` skips standard authentication flow controls.
**Prevention:** Consistently use the standard `ensureAdmin(req, res)` or similar encompassing middleware at the very beginning of administrative routes to strictly enforce both authentication and authorization, ensuring uniform and secure API behavior.
