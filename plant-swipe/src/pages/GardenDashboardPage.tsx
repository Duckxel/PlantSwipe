...
import { Info, ArrowUpRight, Sparkles, CalendarClock } from "lucide-react";
...
export const GardenDashboardPage: React.FC = () => {
  ...
  const heroTitle = t("gardenDashboard.hero.title", {
    name: garden?.name ?? "...",
  });
  const heroSubtitle = t("gardenDashboard.hero.subtitle");
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-0 pb-16 space-y-10">
        ...
        <div className="relative space-y-6 p-8 md:p-12">
          ...
          <div className="flex flex-wrap gap-3 pt-1.5">
            <Button
              ...
              onClick={() => {
                if (id && garden) navigate(`/garden/${id}/plants`);
              }}
              disabled={!garden}
            >
            ...
            <Button
              ...
              className="rounded-2xl"
              onClick={() => {
                if (id && garden) navigate(`/garden/${id}/routine`);
              }}
              disabled={!garden}
            >
      ...
      <div
        className={`grid grid-cols-1 ${user ? "lg:grid-cols-[260px_1fr]" : "md:grid-cols-[260px_1fr]"} gap-8`}
      >
        {loading && (
          <>
            <aside className="space-y-6 md:sticky md:top-6 self-start">
              <section className="rounded-[28px] border border-stone-200/70 dark:border-[#3e3e42]/70 bg-white/75 dark:bg-[#1f1f1f]/75 backdrop-blur px-6 py-6 shadow-sm space-y-4">
                <div className="h-7 w-32 rounded-full bg-stone-200 dark:bg-[#3e3e42] animate-pulse" />
                <div className="space-y-2">
                  {navItems.map((_, idx) => (
                    <div
                      key={idx}
                      className="h-10 w-full rounded-2xl bg-stone-200 dark:bg-[#3e3e42] animate-pulse"
                    />
                  ))}
                </div>
              </section>
            ...
            <main className="min-h-[60vh]">
              {tab === "overview" ? (
                ...
                <div className="rounded-[28px] border border-stone-200/70 dark:border-[#3e3e42]/70 bg-white/70 dark:bg-[#1f1f1f]/70 backdrop-blur px-6 py-6 text-sm text-stone-600 dark:text-stone-300 animate-pulse">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {plants.map((gp: any, idx: number) => (
                          <Card
                            key={gp.id}
                            className={`rounded-2xl overflow-hidden relative ${dragIdx === idx ? "ring-2 ring-black" : ""}`}
                            draggable
                            onDragStart={(e) => {
                            ...
                            <div className="grid grid-cols-3 items-stretch gap-0">
                              ...
                              <div className="col-span-2 p-3">
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="text-xs opacity-60">
                                    {t(
                                      "gardenDashboard.plantsSection.dueToday",
                                    )}{" "}
            ...
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              <DialogContent className="rounded-2xl">
                ...
                <div className="space-y-3">
                  <Input
                    placeholder={t(
                      "gardenDashboard.plantsSection.searchPlants",
                    )}
                    value={plantQuery}
        ...
        {!loading && !garden && !error && (
          <div className="rounded-[28px] border border-stone-200/70 dark:border-[#3e3e42]/70 bg-white/75 dark:bg-[#1f1f1f]/75 backdrop-blur px-6 py-6 text-sm text-stone-600 dark:text-stone-300">
            {t("garden.failedToLoad")}
          </div>
        )}
      </div>
    </div>
