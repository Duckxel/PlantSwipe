    ...
    const savePlant = async (plantOverride?: Plant, options?: { skipOnSaved?: boolean }) => {
      ...
      try {
        ...
        if (isEnglish) {
          ...
          // Reuses the same flat field reading pattern as the English payload
          const pn = plantToSave as any
          const nonEnglishUpdatePayload = {
            status: normalizedStatus,
            admin_commentary: pn.adminCommentary || pn.meta?.adminCommentary || null,
            updated_by: updatedByValue,
            updated_time: new Date().toISOString(),
            scientific_name_species: pn.scientificNameSpecies || pn.identity?.scientificName || null,
            scientific_name_variety: pn.scientificNameVariety || null,
            family: pn.family || pn.identity?.family || null,
            encyclopedia_category: pn.encyclopediaCategory || [],
            featured_month: pn.featuredMonth || [],
            climate: pn.climate || [],
            season: seasonEnum.toDbArray(pn.season || pn.identity?.season),
            utility: utilityEnum.toDbArray(pn.utility),
            edible_part: ediblePartEnum.toDbArray(pn.ediblePart || pn.comestiblePart),
            thorny: coerceBoolean(pn.thorny ?? pn.identity?.spiked, false),
            toxicity_human: toxicityEnum.toDb(pn.toxicityHuman || pn.identity?.toxicityHuman) || null,
            toxicity_pets: toxicityEnum.toDb(pn.toxicityPets || pn.identity?.toxicityPets) || null,
            poisoning_method: pn.poisoningMethod || [],
            life_cycle: lifeCycleEnum.toDbArray(pn.lifeCycle || []),
            average_lifespan: pn.averageLifespan || [],
            foliage_persistence: foliagePersistenceEnum.toDbArray(pn.foliagePersistence || []),
            living_space: livingSpaceEnum.toDbArray(pn.livingSpace || []),
            landscaping: pn.landscaping || [],
            plant_habit: pn.plantHabit || [],
            multicolor: coerceBoolean(pn.multicolor, false),
            bicolor: coerceBoolean(pn.bicolor, false),
            care_level: careLevelEnum.toDbArray(pn.careLevel || []),
            sunlight: sunlightEnum.toDbArray(pn.sunlight || []),
            temperature_max: pn.temperatureMax || pn.plantCare?.temperatureMax || null,
            temperature_min: pn.temperatureMin || pn.plantCare?.temperatureMin || null,
            temperature_ideal: pn.temperatureIdeal || pn.plantCare?.temperatureIdeal || null,
            watering_frequency_warm: pn.wateringFrequencyWarm || null,
            watering_frequency_cold: pn.wateringFrequencyCold || null,
            watering_type: wateringTypeEnum.toDbArray(pn.wateringType || []),
            hygrometry: pn.hygrometry || null,
            misting_frequency: pn.mistingFrequency || null,
            substrate: pn.substrate || [],
            substrate_mix: pn.substrateMix || [],
            mulching_needed: coerceBoolean(pn.mulchingNeeded, false),
            mulch_type: pn.mulchType || [],
            nutrition_need: pn.nutritionNeed || [],
            fertilizer: pn.fertilizer || [],
            sowing_month: pn.sowingMonth || [],
            flowering_month: pn.floweringMonth || [],
            fruiting_month: pn.fruitingMonth || [],
            height_cm: pn.heightCm || null,
            wingspan_cm: pn.wingspanCm || null,
            staking: coerceBoolean(pn.staking, false),
            division: divisionEnum.toDbArray(pn.division || []),
            cultivation_mode: pn.cultivationMode || [],
            sowing_method: sowingMethodEnum.toDbArray(pn.sowingMethod || []),
            transplanting: coerceBoolean(pn.transplanting, null),
            pruning: coerceBoolean(pn.pruning, false),
            pruning_month: pn.pruningMonth || [],
            conservation_status: conservationStatusEnum.toDbArray(pn.conservationStatus || []),
            ecological_status: pn.ecologicalStatus || [],
            biotopes: pn.biotopes || [],
            urban_biotopes: pn.urbanBiotopes || [],
            ecological_tolerance: ecologicalToleranceEnum.toDbArray(pn.ecologicalTolerance || []),
            biodiversity_role: pn.biodiversityRole || [],
            pollinators_attracted: pn.pollinatorsAttracted || [],
            birds_attracted: pn.birdsAttracted || [],
            mammals_attracted: pn.mammalsAttracted || [],
            ecological_management: pn.ecologicalManagement || [],
            ecological_impact: ecologicalImpactEnum.toDbArray(pn.ecologicalImpact || []),
            infusion: coerceBoolean(pn.infusion, false),
            infusion_parts: pn.infusionParts || [],
            medicinal: coerceBoolean(pn.medicinal, false),
            aromatherapy: coerceBoolean(pn.aromatherapy, false),
            fragrance: coerceBoolean(pn.fragrance, false),
            edible_oil: pn.edibleOil || null,
            companion_plants: pn.companionPlants || [],
          }
