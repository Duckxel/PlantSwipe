...
import { User, Loader2, Check, ArrowLeft, Mail } from "lucide-react"
...
export function ForgotUsernamePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.trim())) {
      setError(t('forgotUsername.invalidEmail', 'Please enter a valid email address.'))
      return
    }
    ...
    try {
      ...
      if (!response.ok || !data.found) {
        setError(data.error || t('forgotUsername.noAccountFound', 'No account found with this email address.'))
        setLoading(false)
        return
      }
      ...
      if (data.found && data.sent) {
        ...
        setError(data.reason || t('forgotUsername.failedToSend', 'Failed to send email. Please try again.'))
      }
    } catch (err) {
      console.error('[forgot-username] Error:', err)
      setError(t('forgotUsername.unexpectedError', 'An unexpected error occurred. Please try again.'))
    } finally {
      setLoading(false)
    }
  }
          ...
          <AnimatePresence mode="wait">
            {success ? (
              ...
              >
                ...
                >
                  {t('forgotUsername.successTitle', 'Email Sent!')}
                </motion.h2>
                ...
                >
                  {t('forgotUsername.successDescription', 'We have sent an email with your username. Please check your inbox.')}
                </motion.p>
                ...
                >
                  ...
                  >
                    {t('forgotUsername.backToLogin', 'Back to Login')}
                  </Button>
                </motion.div>
              </motion.div>
            ) : (
              ...
              >
                ...
                >
                  {t('forgotUsername.title', 'Forgot Your Username?')}
                </motion.h1>
                ...
                >
                  {t('forgotUsername.description', 'Enter the email address associated with your account and we will send you your username.')}
                </motion.p>
                ...
                >
                  <div className="grid gap-2 text-left">
                    <Label htmlFor="forgot-username-email">
                      {t('forgotUsername.emailLabel', 'Email Address')}
                    </Label>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        type="email"
                        placeholder={t('forgotUsername.emailPlaceholder', 'you@example.com')}
                        value={email}
                  ...
                  >
                    {loading ? (
                      <>
                        ...
                        {t('forgotUsername.sending', 'Sending...')}
                      </>
                    ) : (
                      t('forgotUsername.submit', 'Send Username Reminder')
                    )}
                  </Button>
                </motion.div>
                ...
                >
                  {t('forgotUsername.checkSpam', "Check your spam folder if you don't see the email.")}
                </motion.p>
              </motion.div>
            )}
          </AnimatePresence>
