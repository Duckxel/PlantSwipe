        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-stretch md:min-h-[calc(100vh-96px)]">
          ...
          <main className="flex-1 w-full">
            ...
            {(apiProbe.ok === false ||
              ...
              <Card className="relative overflow-hidden rounded-[32px] border border-stone-200 dark:border-[#3e3e42] bg-gradient-to-br from-emerald-50 via-white to-stone-100 dark:from-[#252526] dark:via-[#1e1e1e] dark:to-[#16161a] shadow-[0_35px_60px_-20px_rgba(16,185,129,0.35)]">
                <div className="absolute inset-0 pointer-events-none">
                  <div className="absolute -top-16 -right-10 h-48 w-48 rounded-full bg-emerald-200/50 dark:bg-emerald-500/10 blur-3xl" />
                  <div className="absolute bottom-0 left-0 h-48 w-48 rounded-full bg-emerald-100/40 dark:bg-emerald-500/10 blur-3xl" />
                </div>
                <CardContent className="relative z-10 p-6 md:p-8 space-y-6">
                  <div className="flex flex-col gap-3">
                    <Badge className="w-fit rounded-2xl px-3 py-1 bg-white/80 dark:bg-[#2c2c2f]/70 backdrop-blur text-xs tracking-wide">
                      Journal equipe
                    </Badge>
                    <div className="text-2xl font-semibold tracking-tight">
                      Deux gars, zero panneau "travaux"
                    </div>
                    <p className="text-sm text-stone-600 dark:text-stone-300 max-w-3xl">
                      L'un reve de lignes de semis, l'autre reve de lignes de code. Ensemble on veut aider les gens a lancer leur mini agriculture urbaine,
                      a gerer l'arrosage sans stress et a manger de bons fruits et legumes faits maison.
                    </p>
                  </div>
                  <div className="grid gap-4 md:grid-cols-3">
                    {adminStoryBeats.map((story) => (
                      <div
                        key={story.title}
                        className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/80 dark:bg-[#1b1b1f]/60 backdrop-blur p-4"
                      >
                  ...
                  <div className="grid gap-4 lg:grid-cols-3">
                    <div className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/90 dark:bg-[#1c1c20]/70 p-4 space-y-3">
                      <div className="text-sm font-semibold">Ce qu'on propose</div>
                      <ul className="space-y-2 text-sm">
                        {adminOfferingList.map((item) => (
                          <li
                            key={item.label}
                            className="flex items-center justify-between gap-3 text-stone-700 dark:text-stone-200"
                          >
                            <span>{item.label}</span>
                            {item.status && (
                              <Badge
                                variant={item.status === "Maintenant" ? "secondary" : "outline"}
                    ...
                    <div className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/90 dark:bg-[#1c1c20]/70 p-4 space-y-3">
                      <div className="text-sm font-semibold">Services a venir</div>
                      <ul className="space-y-2 text-sm text-stone-600 dark:text-stone-300">
                        {adminUpcomingServices.map((item) => (
                    ...
                    <div className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/90 dark:bg-[#1c1c20]/70 p-4 space-y-4">
                      <div className="space-y-2">
                        <div className="text-sm font-semibold">Information pratique</div>
                        <p className="text-sm text-stone-600 dark:text-stone-300">
                          {adminInfoCopy.summary}
                        </p>
                      </div>
                      <div className="border-t border-dashed border-stone-200 dark:border-[#3e3e42]/60 pt-4 space-y-2">
                        <div className="text-sm font-semibold">Collaboration</div>
                        <p className="text-sm text-stone-600 dark:text-stone-300">
                          {adminInfoCopy.collaboration}
                        </p>
                      </div>
                    </div>
                  </div>
