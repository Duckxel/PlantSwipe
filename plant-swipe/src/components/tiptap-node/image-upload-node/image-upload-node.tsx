...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const insertImageNodes = useCallback(
    (urls: string[], labels?: Array<string | undefined>) => {
      const pos = props.getPos()
      if (!isValidPosition(pos) || urls.length === 0) return

      const imageNodes = urls.map((url, index) => {
        const providedLabel = labels?.[index]?.trim()
        const derivedFromUrl = url
          .split("/")
          .pop()
          ?.replace(/\?.*$/, "")
          ?.replace(/\.[^/.]+$/, "")
        const fallback = providedLabel || derivedFromUrl || "image"
        return {
  ...
  const handleInsertFromLink = () => {
    ...
    if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
      setLinkError("Only http(s) URLs are supported")
      return
    }
    insertImageNodes([trimmed], [altValue.trim() || undefined])
    setLinkValue("")
    setAltValue("")
    setLinkError(null)
    setMode("upload")
      ...
      {mode === "link" && (
        ...
        >
          <div className="tiptap-image-upload-link-field">
            <label htmlFor="tiptap-image-url">Image URL</label>
            <input
              id="tiptap-image-url"
              type="url"
          ...
          <div className="tiptap-image-upload-link-field">
            <label htmlFor="tiptap-image-alt">Alt text (optional)</label>
            <input
              id="tiptap-image-alt"
              type="text"
          ...
          {linkError && (
            <p className="tiptap-image-upload-link-error">{linkError}</p>
          )}
          <div className="tiptap-image-upload-link-actions">
            <Button
              type="button"
              data-style="primary"
