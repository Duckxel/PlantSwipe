...
/** Convert a list of preview canvases to PNG blobs, skipping any tainted ones. */
export async function canvasesToCardBlobs(
  ...
  for (let i = 0; i < canvases.length; i++) {
    ...
    if (!c) continue
    try {
      blobs.push(await canvasToBlob(c, "image/png"))
    } catch (e) {
...
export type AdminExportBufferScheduleProps = {
  ...
  caption: string
  /** Returns the image blobs (PNG/JPEG) to attach to the post, in order. */
  getCardBlobs: () => Promise<Blob[]>
  /** Approximate number of cards that will be attached (UI only). */
  cardCount: number
...
export const AdminExportBufferSchedule: React.FC<AdminExportBufferScheduleProps> = ({
  ...
  const handleSchedule = async () => {
    ...
    let blobs: Blob[] = []
    try {
      blobs = await getCardBlobs()
    } catch (e) {
      setSubmitError(e instanceof Error ? e.message : "Failed to prepare card images")
      setSubmitting(false)
      return
    }
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-stone-400 mt-0.5">
            Sends the caption{plantName ? ` for ${plantName}` : ""} to Instagram, Facebook, or Twitter via Buffer, with the {cardCount} attached card{cardCount === 1 ? "" : "s"} as media.
          </div>
        </div>
        <Button
          type="button"
