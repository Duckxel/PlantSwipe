...
export function TaskCreateDialog({
  ...
  const save = async () => {
    ...
    if (countSelected !== amount) {
      const unit = period === 'week' ? t('garden.taskDialog.daysPerWeek') : period === 'month' ? t('garden.taskDialog.daysPerMonth') : t('garden.taskDialog.timesPerYear')
      setError(t('garden.taskDialog.selectExactly', { amount, period: unit }))
      return
    }
    setSaving(true)
    try {
      ...
      try {
        ...
        await logGardenActivity({ gardenId, kind: 'note' as any, message: t('garden.taskDialog.addedTask', { taskName: taskTypeLabel }), taskName: taskTypeLabel, actorColor: null })
      } catch {}
      ...
      setError(e?.message || t('garden.taskDialog.failedToCreate'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle>{t('garden.taskDialog.createTask')}</DialogTitle>
          <DialogDescription>{t('garden.taskDialog.createTaskDescription')}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {(['week','month','year'] as Period[]).map(p => (
                  <option key={p} value={p} className="capitalize">{t(`garden.taskDialog.${p}`)}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          {type === 'custom' && (
            <div className="grid gap-2">
              <label className="text-sm font-medium">{t('garden.taskDialog.customTaskName')}</label>
              <Input value={customName} onChange={(e: any) => setCustomName(e.target.value)} placeholder={t('garden.taskDialog.customTaskNamePlaceholder')} />
            </div>
          )}

          {type === 'custom' && (
            <div className="grid gap-2">
              <label className="text-sm font-medium">{t('garden.taskDialog.emoji')}</label>
              <div className="flex items-center gap-2">
                <Input value={emoji} onChange={(e: any) => setEmoji(e.target.value)} placeholder={t('garden.taskDialog.emojiPlaceholder')} maxLength={4} />
                <div className="text-sm opacity-60">{t('garden.taskDialog.optional')}</div>
              </div>
          ...
          <div className="text-sm opacity-60">
            {period === 'week' && t('garden.taskDialog.pickDaysWeek')}
            {period === 'month' && t('garden.taskDialog.pickDaysMonth')}
            {period === 'year' && t('garden.taskDialog.pickDaysYear')}
          </div>
          ...
          {period === 'month' && (
            <>
              <div className="text-xs opacity-70">{t('garden.taskDialog.pickWeeksExample')}</div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" className="rounded-2xl" onClick={() => onOpenChange(false)} disabled={saving}>{t('garden.taskDialog.close')}</Button>
            <Button className="rounded-2xl" onClick={save} disabled={saving || countSelected !== amount}>{saving ? t('garden.taskDialog.creating') : t('garden.taskDialog.createTaskButton')}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function WeekPicker({ selectedNumbers, onToggleNumber, disabledMore }: { selectedNumbers: number[]; onToggleNumber: (uiIndex: number) => void; disabledMore: boolean }) {
  ...
  const display = [
    ...
    { label: t('garden.taskDialog.dayLabels.wed'), uiIndex: 2 },
    { label: t('garden.taskDialog.dayLabels.thu'), uiIndex: 3 },
    { label: t('garden.taskDialog.dayLabels.fri'), uiIndex: 4 },
    { label: t('garden.taskDialog.dayLabels.sat'), uiIndex: 5 },
    { label: t('garden.taskDialog.dayLabels.sun'), uiIndex: 6 },
  ]
...
function MonthNthWeekdayPicker({ selected, onToggle, onToggleHeader, disabledMore }: { selected: string[]; onToggle: (weekIndex: number, uiIndex: number) => void; onToggleHeader: (uiIndex: number) => void; disabledMore: boolean }) {
  ...
  const labels = [
    ...
    t('garden.taskDialog.dayLabels.wed'),
    t('garden.taskDialog.dayLabels.thu'),
    t('garden.taskDialog.dayLabels.fri'),
    t('garden.taskDialog.dayLabels.sat'),
    t('garden.taskDialog.dayLabels.sun')
  ]
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-[60px_repeat(7,minmax(0,1fr))] gap-2 items-center">
        <div className="text-xs opacity-70 text-center">{t('garden.taskDialog.weekLabel')}</div>
...
function YearMonthNthWeekdayPicker({ selected, onToggle, onToggleHeader, disabledMore }: { selected: string[]; onToggle: (monthIdx: number, weekIndex: number, uiIndex: number) => void; onToggleHeader: (monthIdx: number, uiIndex: number) => void; disabledMore: boolean }) {
  ...
  const months = [
    ...
    t('garden.taskDialog.monthNames.mar'),
    t('garden.taskDialog.monthNames.apr'),
    t('garden.taskDialog.monthNames.may'),
    t('garden.taskDialog.monthNames.jun'),
    t('garden.taskDialog.monthNames.jul'),
    t('garden.taskDialog.monthNames.aug'),
    t('garden.taskDialog.monthNames.sep'),
    t('garden.taskDialog.monthNames.oct'),
    t('garden.taskDialog.monthNames.nov'),
    t('garden.taskDialog.monthNames.dec')
  ]
  const labels = [
    t('garden.taskDialog.dayLabels.mon'),
    t('garden.taskDialog.dayLabels.tue'),
