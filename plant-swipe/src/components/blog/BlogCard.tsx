...
import { CalendarClock, Pencil, UserRound, ArrowRight } from 'lucide-react'
...
export const BlogCard: React.FC<BlogCardProps> = ({ post, isAdmin, onEdit }) => {
  ...
  return (
    <Link to={`/blog/${articleTarget}`} className="block group">
      <Card className="rounded-3xl overflow-hidden border border-stone-200 dark:border-[#3e3e42] flex flex-col bg-white/80 dark:bg-[#1f1f1f] shadow-sm hover:shadow-lg transition-all duration-200 group-hover:border-emerald-400/50 dark:group-hover:border-emerald-500/40 h-full cursor-pointer">
        <div className="relative h-48 bg-stone-100 dark:bg-[#2b2b2b] overflow-hidden">
          {displayImageUrl ? (
            <img
              src={displayImageUrl}
              alt={post.title}
        ...
        <CardHeader className="space-y-2">
          ...
          <h3 className="text-xl font-semibold group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{post.title}</h3>
