...
export default function PlantSwipe() {
  ...
  const preparedPlants = useMemo(() => {
    ...
    return plants.map((p) => {
      ...
      // Base object with eager properties
      const prepared = {
        ...p,
      ...
      } as any // Use any temporarily to allow defineProperties

      // Lazy-load expensive properties
      // These are only computed when accessed (e.g. during filtering)
      Object.defineProperties(prepared, {
        _searchString: {
          get() {
            // Lazy initialization pattern: compute, then replace the getter with the value
            ...
            const value = `${p.name} ${p.scientificName || ''} ${p.meaning || ''} ${colors.join(" ")} ${commonNames} ${synonyms} ${givenNames}`.toLowerCase()
        ...
        _colorTokens: {
          get() {
            // Access _normalizedColors (triggering its getter if needed)
            const normalizedColors = this._normalizedColors as string[]
            const tokens = new Set<string>()
        ...
        _usageSet: {
          get() {
            const value = new Set(this._usageLabels as string[])
        ...
        _habitatSet: {
          get() {
            const value = new Set(this._habitats as string[])
  ...
  const handleAcceptCookiesForAuth = React.useCallback(() => {
    try {
      ...
      setAuthError(null)
      // Dispatch event for reCAPTCHA loading
      window.dispatchEvent(new CustomEvent('cookie_consent_granted', { detail: { level: 'essential' } }))
    } catch {
      // Ignore localStorage errors
    }
  }, [])
          ...
          <Dialog open={authOpen && !user} onOpenChange={setAuthOpen}>
            <DialogContent className="rounded-2xl">
              ...
              <div className="space-y-3">
                {/* Cookie acceptance requirement notice */}
                {authNeedsCookies && (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-medium text-amber-800 dark:text-amber-200">
                          {t('auth.cookiesRequiredTitle', 'Cookies Required')}
                        </h4>
                        <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">
                          {t('auth.cookiesRequiredDescription', 'To protect your account, we use reCAPTCHA which requires cookies. Please accept cookies to continue.')}
                        </p>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                          className="mt-3 rounded-xl border-amber-300 dark:border-amber-600 text-amber-700 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-800/30"
                          onClick={handleAcceptCookiesForAuth}
                        >
                          {t('auth.acceptCookiesButton', 'Accept Essential Cookies')}
                        </Button>
                      </div>
                    </div>
                  </div>
