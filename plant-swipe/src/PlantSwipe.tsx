...
export default function PlantSwipe() {
  ...
  // Pre-calculate search strings and normalized attributes to avoid doing it on every render/filter
  const preparedPlants = useMemo(() => {
    return plants.map((p) => {
      ...
      // 4. Usage Labels
      const usageLabels = getPlantUsageLabels(p).map(l => l.toLowerCase())
      ...
      return {
        ...
        _prepared: {
          searchString,
          normalizedColors: colors.map(c => c.toLowerCase().trim()),
          typeLabel,
          usageLabels,
          habitats,
          maintenance,
  ...
  const filtered = useMemo(() => {
    ...
    return preparedPlants.filter((p: PreparedPlant) => {
      ...
      const matchesQ = p._prepared.searchString.includes(lowerQuery)
      ...
      // Optimized color matching using pre-calculated normalized colors
      const matchesColor = expandedColorFilters.length === 0 
        ? true 
        : expandedColorFilters.some((filterColor) => 
            p._prepared.normalizedColors.some((plantColorNormalized) => {
               // simplified colorMatches logic since we already have normalized plant color
               if (plantColorNormalized === filterColor) return true

               // Check tokens if no direct match
               const tokens = plantColorNormalized
                 .replace(/[-_/]+/g, " ")
                 .split(/\s+/)
                 .filter(Boolean)
               return tokens.includes(filterColor)
            })
          )
      ...
      const matchesType = normalizedType ? p._prepared.typeLabel === normalizedType : true

      const matchesUsage = normalizedUsage.length
        ? normalizedUsage.every((usage) => p._prepared.usageLabels.includes(usage))
        : true
      ...
      if (livingSpaceFilters.length === 2) {
        ...
        matchesLivingSpace = p._prepared.livingSpace === 'both'
      } else if (livingSpaceFilters.length === 1) {
        const selectedSpace = livingSpaceFilters[0].toLowerCase()
        if (selectedSpace === 'indoor') {
          matchesLivingSpace = p._prepared.livingSpace === 'indoor' || p._prepared.livingSpace === 'both'
        } else if (selectedSpace === 'outdoor') {
          matchesLivingSpace = p._prepared.livingSpace === 'outdoor' || p._prepared.livingSpace === 'both'
        }
      }
