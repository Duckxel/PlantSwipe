...
-- Function: Enriched occurrences for multiple gardens in a single query
-- Joins occurrences with task metadata (type, emoji) server-side so the
-- client doesn't need to fetch tasks + occurrences + merge.
