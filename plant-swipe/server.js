...
// Security headers middleware - includes CSP that allows Sentry error tracking
app.use((req, res, next) => {
  ...
  try {
    // Content Security Policy - allows necessary external services
    // Sentry: *.sentry.io, *.ingest.de.sentry.io (error tracking)
    // Supabase: *.supabase.co (database/storage/auth)
    // Google: reCAPTCHA, Analytics, Fonts
    // Media: media.aphylia.app (our CDN)
    const cspDirectives = [
      // Default: only allow from same origin
      "default-src 'self'",
      
      // Scripts: self, inline (for Vite HMR in dev), Google services
      ...
      // Images: self, data URIs, blobs, Supabase storage, our media CDN
      "img-src 'self' data: blob: https://*.supabase.co https://media.aphylia.app",
      
      // Connect (XHR/fetch/WebSocket): self, Supabase, Sentry, Google
      "connect-src 'self' https://*.supabase.co wss://*.supabase.co https://*.sentry.io https://*.ingest.de.sentry.io https://*.ingest.sentry.io https://www.google.com https://www.google-analytics.com https://www.googletagmanager.com",
      
      // Frames: self, Google reCAPTCHA
      "frame-src 'self' https://www.google.com https://recaptcha.google.com",
      
      // Workers: self (for service worker)
      "worker-src 'self' blob:",
      
      // Manifests: self (for PWA manifest)
      "manifest-src 'self'",
      
      // Media (audio/video): self, Supabase
      "media-src 'self' https://*.supabase.co https://media.aphylia.app",
